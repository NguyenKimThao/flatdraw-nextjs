...
const useCollectionApi = create<{
  ...
  createCollection: async (name: string, desc: string) => {
    ...
    const data = await ApiService.createCollection(name, '{}', desc);
    if (data == null || data.error != 0 || !data.data) {
      set((state) => ({
        ...state,
        status: StatusFetch.FAIL,
  ...
  deleteCollection: async (id: number) => {
    ...
    const data = await ApiService.deleteCollection(id);
    if (data == null || data.error != 0 || !data.data) {
      set((state) => ({
        ...state,
        status: StatusFetch.FAIL,
      ...
      const collections = get().collections.filter(e => e.id != id);
      set((state) => ({
        ...state,
        status: StatusFetch.SUCCESS,
